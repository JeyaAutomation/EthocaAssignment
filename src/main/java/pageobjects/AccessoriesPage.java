...
public class AccessoriesPage extends BasePage {
	...
	public void addAnItemToCart(Item item, ProductView view, int quantity) {
		...
		String itemContainer = view == ProductView.list
				? String.format("div.default_product_display.%s.accessories.group", item.getItemReference())
				: String.format("div.product_grid_item.%s", item.getItemReference());
		...
		for (int i = 0; i < quantity; i++) {
			...
			SeleniumUtil.waitForElementToDisappear(driver, this .driver.findElement(By.cssSelector("div.wpsc_loading_animation")));
